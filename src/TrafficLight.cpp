...
T MessageQueue<T>::receive()
{
    ...
    return std::move(msg); // will not be copied due to return value optimization (RVO) in C++
}
...
void TrafficLight::waitForGreen()
{
    ...
    TrafficLightPhase objTLP;
    
    while(true)
    {
        std::this_thread::sleep_for(std::chrono::milliseconds(1));

        objTLP = this->_queueTLP.receive();

        if( objTLP == TrafficLightPhase::green )
          return;
    }
}
...
void TrafficLight::cycleThroughPhases()
{
    ...
    //double cycleDuration = rand() % 6 + 4; // duration of a single simulation cycle b/w 4 & 6 seconds
    ...
    while (true)
    {
        ...
        if (timeSinceLastUpdate > cycleDuration)
        {
            ...
            _queueTLP.send(std::move(_currentPhase));

            // reset the lastupdate value
            lastUpdate = std::chrono::system_clock::now();
